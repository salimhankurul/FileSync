...
static const wchar_t *SZ_AGENT = L"blackmamba";
...
public:
	...
	inline bool RunSecurityChecks();
    inline bool SendHttpRequest(const wstring &httpVerb = L"GET", bool disableAutoRedirect = false, bool CalculatePersantage = false);
...
bool WinHttpClient::RunSecurityChecks() {
	//SecurityManager::GetInstance()->CheckHooked(WinHttpOpen, META_WSTRING(L"WinHttpOpen"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpSetTimeouts, META_WSTRING(L"WinHttpSetTimeouts"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpCrackUrl, META_WSTRING(L"WinHttpCrackUrl"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpConnect, META_WSTRING(L"WinHttpConnect"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpOpenRequest, META_WSTRING(L"WinHttpOpenRequest"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpSetOption, META_WSTRING(L"WinHttpSetOption"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpAddRequestHeaders, META_WSTRING(L"WinHttpAddRequestHeaders"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpSetOption, META_WSTRING(L"WinHttpSetOption"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpSendRequest, META_WSTRING(L"WinHttpSendRequest"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpGetIEProxyConfigForCurrentUser, META_WSTRING(L"WinHttpGetIEProxyConfigForCurrentUser"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpGetProxyForUrl, META_WSTRING(L"WinHttpGetProxyForUrl"));
	//SecurityManager::GetInstance()->CheckHooked(GlobalFree, META_WSTRING(L"GlobalFree"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpReceiveResponse, META_WSTRING(L"WinHttpReceiveResponse"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpQueryHeaders, META_WSTRING(L"WinHttpQueryHeaders"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpQueryDataAvailable, META_WSTRING(L"WinHttpQueryDataAvailable"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpQueryDataAvailable, META_WSTRING(L"WinHttpQueryDataAvailable"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpReadData, META_WSTRING(L"WinHttpReadData"));
	//SecurityManager::GetInstance()->CheckHooked(MultiByteToWideChar, META_WSTRING(L"MultiByteToWideChar"));
	//SecurityManager::GetInstance()->CheckHooked(WinHttpCloseHandle, META_WSTRING(L"WinHttpCloseHandle"));

	return true;
}

bool WinHttpClient::SendHttpRequest(const wstring &httpVerb, bool disableAutoRedirect,bool CalculatePersantage)
{
	this->RunSecurityChecks();

	auto printProg = [](int x) -> std::string 
	{
		std::string s;
		s = "[";
    ...
    if (_wcsicmp(verb.c_str(), META_WSTRING(L"GET")) == 0)
    {
        verb = META_WSTRING(L"GET");
    }
    else if (_wcsicmp(verb.c_str(), META_WSTRING(L"POST")) == 0)
    {
        verb = META_WSTRING(L"POST");
    }
    else
    {
        ...
        if (hConnect != NULL)
        {
            ...
            if (hRequest != NULL)
            {
                ...
                while (!bGetReponseSucceed && iRetryTimes++ < INT_RETRYTIMES)
                {
                    ...
                    {
                        wstring cookies = META_WSTRING(L"Cookie: ");
                    ...
                    if (bSendRequestSucceed)
                    {
                        ...
                        if (::WinHttpReceiveResponse(hRequest, NULL))
                        {
                            ...
                            if (bResult || (!bResult && (::GetLastError() == ERROR_INSUFFICIENT_BUFFER)))
                            {
                                ...
                                if (szHeader != NULL)
                                {
                                                              ...
                                                              WINHTTP_NO_HEADER_INDEX))
                                    {
                                        ...
                                        if (!m_bForceCharset)
                                        {
                                            regExp = META_WSTRING(L"charset={[A-Za-z0-9\\-_]+}");
                                            if (ParseRegExp(regExp, false, 1, m_responseHeader, result) && result.size() > 0)
                                            {
                                                m_responseCharset = result[0];
                                            }
                                        }
                                        regExp = META_WSTRING(L"Content-Length: {[0-9]+}");
                                        if (ParseRegExp(regExp, false, 1, m_responseHeader, result) && result.size() > 0)
                                        {
                                            m_responseByteCount = (unsigned int)_wtoi(result[0].c_str());
                                           // std::wcout << m_responseByteCount << endl;
                                        }
                                        regExp = META_WSTRING(L"Location: {[0-9]+}");
                                        if (ParseRegExp(regExp, false, 1, m_responseHeader, result) && result.size() > 0)
                                        {
                                            m_location = result[0];
                                        }
                                        regExp = META_WSTRING(L"Set-Cookie:\\b*{.+?}\\n");
                                        if (ParseRegExp(regExp, false, 1, m_responseHeader, result) && result.size() > 0)
                                        {
                                           ...
                                           // std::wcout << m_responseCookies << endl;
                                ...
                                if (::WinHttpQueryDataAvailable(hRequest, &dwSize))
                                {
                                    ...
                                    if (pResponse != NULL)
                                    {
                                                              ...
                                                              &dwRead))
                                        {
											...
											if (CalculatePersantage)
											{
												int Persantage = (int)((float)iCurrentBufferSize / (float)m_responseByteCount * 100.f);
												std::cout << META_STRING("\r") << setw(-20) << printProg(Persantage) << META_STRING(" ") << Persantage << META_STRING("% completed.") << flush;
                            ...
                            if (_wcsnicmp(m_responseCharset.c_str(), META_WSTRING(L"utf-8"), 5) == 0)
                            {
                                codePage = CP_UTF8;
                                dwFlag = 0;
                            }
                            ...
                            if (verb == META_WSTRING(L"GET") && !disableAutoRedirect)
                            {
                                wstring regExp = META_WSTRING(L"{<html>}");
                                vector<wstring> result;
                                if (ParseRegExp(regExp, false, 1, m_responseContent, result) && result.size() > 0)
                                {
                                    regExp = META_WSTRING(L"{</html>}");
                                    if (!ParseRegExp(regExp, false, 1, m_responseContent, result) || result.size() <= 0)
                                    {
...
bool WinHttpClient::SaveResponseToFile(const wstring &filePath)
{
    ...
    FILE *f = NULL;
    int iResult = _wfopen_s(&f, filePath.c_str(), META_WSTRING(L"wb"));
    if (iResult == 0 && f != NULL)
    {
